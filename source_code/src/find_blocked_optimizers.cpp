							  ...
							  int &dead_b, int &dead_u, int &alive_b, int &alive_u, int &num_iterations)
{
	...
	auto find_neighbour_power = [&] () // target reaction
	{
		auto find_neighbour_power_impl = [&] (int target, double power, auto &self_ref) -> void
		{
			...
			for(const auto &i:rtos.at(target))
			{
				...
				for(const auto &j:stor.at(i.first))
				{
					...
					if(rpower[reac_intoor.at(j.first)]==-1) self_ref(j.first,stoic2/stoic1,self_ref);
				}
			}
		};
		bool any_undefined = true;
		int start = rtos.begin()->first;
		do // this is done in case the reaction network consists of several disconnected subnetworks
		{
			find_neighbour_power_impl(start,1.0,find_neighbour_power_impl);
			any_undefined = false;
			for(size_t i=0;i<rpower.size();i++) if(rpower[i]==-1) {any_undefined=true; start = reac_ortoin[i]; break;}
		} while(any_undefined);
	};
	find_neighbour_power();
	...
	auto retighten = [&] () // Retighten auxiliaries around all unknown reactions
	{
		for (size_t i = 0; i < rtos.size(); i++) 
		{
	...
	auto retighten_powerful = [&] () // Retighten auxiliaries around the most powerful reaction
	{
	...
	auto flip_reacs = [&] (vector<int> flippable)
	{
...
SINKS_obo::SINKS_obo(map<int,map<int,pair<int,double>>> nrtos, map<int,map<int,pair<int,double>>> nstor) : rtos(nrtos), stor(nstor)
{
	for(const auto &i:rtos) reversible.push_back(i.second.begin()->second.first>1 ? true : false);
	for(const auto &i:stor) spec_intoor.emplace(i.first,spec_intoor.size());
	for(const auto &i:stor) spec_ortoin.push_back(i.first);
	...
	// initialize empty sink 

	vector<int> columnIndex;
	vector<double> columnValue;
	columnIndex.push_back(0);
	columnValue.push_back(0.0);
	linear_model.addColumn((int)columnIndex.size(), &columnIndex[0], &columnValue[0], 0.0, 1.0, 1.0);
}

bool SINKS_obo::get_sink(int num, int dir)
{
 	/// Initialize sink
	linear_model.setElement(spec_intoor.at(num),rtos.size(),(double)dir);
	...
	const double * solution = solver.primalColumnSolution();
	bool ret = (solution[rtos.size()]>0.9) ? true : false;

	// Remove sink
	linear_model.setElement(spec_intoor.at(num),rtos.size(),0.0);
